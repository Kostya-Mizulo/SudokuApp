...
//      rootBundle (реальный ассет, как в рантайме);
...
void main() {
  ...
  test('СИМУЛЯЦИЯ: старт игры MASTER → загрузка из файла → решение решателем',
    ...
    // Загружаем первую головоломку из ассета напрямую через rootBundle.
    final content = await rootBundle
        .loadString('lib/resources/puzzles/master.json');
    final data = jsonDecode(content) as Map<String, dynamic>;
    final puzzleData =
        (data['puzzles'] as List<dynamic>).first as Map<String, dynamic>;

    final solvedGrid = (puzzleData['solvedGrid'] as List<dynamic>)
        .map((row) => (row as List<dynamic>).cast<int>())
        .toList();
    final puzzleGrid = (puzzleData['puzzleGrid'] as List<dynamic>)
        .map((row) => (row as List<dynamic>).cast<int>())
        .toList();
    ...
    final cells = [
      for (var row = 0; row < solvedGrid.length; row++)
        [
          for (var col = 0; col < solvedGrid[row].length; col++)
            Cell(row, col)
              ..setRealNumber(solvedGrid[row][col])
              ..setNumberByStart(puzzleGrid[row][col])
        ]
    ];
    ...
    final emptyCount = puzzleGridFromCells
        .expand((r) => r)
        .where((v) => v == 0)
        .length;
    ...
    for (var r = 0; r < 9; r++) {
      for (var c = 0; c < 9; c++) {
        if (puzzleGridFromCells[r][c] != 0) {
          expect(puzzleGridFromCells[r][c], solvedFromFile[r][c],
    ...
    for (var r = 0; r < 9; r++) {
      for (var c = 0; c < 9; c++) {
        if (puzzleGridFromCells[r][c] != 0) {
          expect(solvedByEngine[r][c], puzzleGridFromCells[r][c]);
        }
      }
    }
